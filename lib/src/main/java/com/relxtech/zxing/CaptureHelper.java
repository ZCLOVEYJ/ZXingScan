...
import com.relxtech.zxing.camera.CameraManager;
import com.relxtech.zxing.camera.FrontLightMode;
...
public class CaptureHelper implements CaptureLifecycle, CaptureTouchEvent, CaptureManager, SurfaceHolder.Callback {
     ...
     * @param surfaceHolder
     */
    private void initCamera(SurfaceView surfaceView, SurfaceHolder surfaceHolder) {
    ...
    public void onResult(Result result, Bitmap barcode) {
        ...
        if (isContinuousScan) {
            if (onCaptureCallback != null) {
                onCaptureCallback.onResultCallback(text, barcode);
            }
        ...
        if (isPlayBeep) {//如果播放音效，则稍微延迟一点，给予播放音效时间
            captureHandler.postDelayed(() -> {
                ...
                if (onCaptureCallback != null && onCaptureCallback.onResultCallback(text, barcode)) {
                    return;
                }
...
//        activity.finish();
    ...
    /**
     * 设置是否连续扫码，如果想支持连续扫码，则将此方法返回{@code true}并重写
     */
